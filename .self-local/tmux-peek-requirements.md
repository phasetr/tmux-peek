...
- 一覧バッファでは refresh, quit, session 削除, session 内容の tail 的な単発表示を行える
- session 内容表示は対象 session の pane 一覧を取得し, 初期実装では先頭 pane を `capture-pane` + `:tail-lines` で取得する
- session 内容表示の既定 `:tail-lines` は enkan-repl 連携で長めの出力を確認できるよう 10000 行とする
...
- `tmux-peek-session-list-tail-lines`: session list UI の tail 的表示で取得する行数. 既定は 10000
...
2026-05-16 時点の macOS / tmux 3.6a 実測では, 約 0.9MB の 10000 行
capture は `tmux capture-pane` が約 0.02 秒で完了し, Emacs 側の
バッファ一括差し替えも問題になる負荷ではなかった.
頻繁な polling ではなく手動の単発確認であることを前提に,
session list UI の既定値は 10000 行にする.
同一 Emacs セッション内で旧版を読み込んだ後に再ロードする場合,
`defcustom` は既に束縛済みの旧既定値 80 を上書きしないため,
未カスタムの 80 だけを 10000 に移行する.
ユーザーが明示的に 80 を custom 保存している場合はその値を尊重する.
