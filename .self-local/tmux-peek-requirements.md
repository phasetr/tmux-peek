...
状態確認と, 余計な pane の削除を行えるようにする.

現状で最も欲しいのは enkan-repl で連携している tmux セッションを Emacs から簡単に確認できる方法である.
立てただけで動かしていないものや異常に増えたものを確認し, 必要に応じて分割した画面領域の一つ, すなわち tmux の pane だけを kill する機能も要件に含める.
...
| 余計な pane を削除する `kill-pane` | ◯ スコープ内. 状態確認と組み合わせる目的で初期実装対象 |
| `kill-session`, `kill-window`, `kill-server` | ✕ 危険なので封印対象. 実装しない |
| 同期呼び出し API | △ 補助. 非同期 API を優先し, 必要になった段階で追加 |
...
| Emacs 側の表示用 UI(専用バッファ, transient メニュー等) | △ 後続課題. コア API とは分離 |
...
- **責務分離**: enkan-repl で対応できる実行・送信・接続系は tmux-peek では作らない. tmux-peek は状態確認・情報取得を優先する. ただし enkan-repl で作られた余計な pane を確認して削除するための `kill-pane` は明確に対象に含める
...
| tmux サブコマンド | elisp 関数(非同期 / 同期補助) | 返り値の大枠 |
|---|---|---|
| `list-sessions` | `tmux-peek-list-sessions-async` / `tmux-peek-list-sessions` | session の構造化リスト |
| `list-windows` | `tmux-peek-list-windows-async` / `tmux-peek-list-windows` | window の構造化リスト |
| `list-panes` | `tmux-peek-list-panes-async` / `tmux-peek-list-panes` | pane の構造化リスト |
| `list-clients` | `tmux-peek-list-clients-async` / `tmux-peek-list-clients` | client の構造化リスト |
| `list-buffers` | `tmux-peek-list-buffers-async` / `tmux-peek-list-buffers` | paste-buffer の構造化リスト |

### 3.2 内容取得系

| tmux サブコマンド | elisp 関数 | 用途 |
|---|---|---|
| `capture-pane -p` | `tmux-peek-capture-pane-async` / `tmux-peek-capture-pane` | ペインの可視内容またはスクロールバックを文字列で取得. 開始/終了行・履歴含むか等をオプションで指定 |
| `show-buffer` | `tmux-peek-show-buffer-async` / `tmux-peek-show-buffer` | 指定ペーストバッファの内容文字列 |

### 3.3 メタ情報取得系

| tmux サブコマンド | elisp 関数 | 用途 |
|---|---|---|
| `display-message -p '<format>'` | `tmux-peek-display-message-async` / `tmux-peek-display-message` | 任意フォーマット文字列を tmux で展開した結果を取得. 全メタ情報取得の最低レイヤ |
| `show-options` | `tmux-peek-show-options-async` / `tmux-peek-show-options` | global / session / window のオプション照会 |
| `show-environment` | `tmux-peek-show-environment-async` / `tmux-peek-show-environment` | 環境変数照会 |

### 3.4 削除系

enkan-repl で異常に作られた, または enkan-repl 側だけでは kill しきれない可能性がある tmux 対象を確認し,
余計な pane を削除するため, `kill-pane` だけを明確にスコープ内とする.
ここでいう pane は tmux で分割した画面領域の一つを指す.
削除系は状態確認 API と組み合わせて使うことを前提にし, 自動判断による削除は行わない.

| tmux サブコマンド | elisp 関数 | 用途 |
|---|---|---|
| `kill-pane` | `tmux-peek-kill-pane-async` | 指定ペイン削除 |

`kill-session`, `kill-window`, `kill-server` は危険度が高いため実装しない.
...
| `kill-session`, `kill-window`, `kill-server` | 危険なので封印対象 |
| `choose-*`, `display-menu`, `display-popup`, `command-prompt` 等の tmux interactive UI 系 | 原則非対応 |

tmux-peek は状態確認・情報取得のための薄い wrapper として設計し,
実行・送信・接続・UI 制御の責務は持たない. `kill-pane` は余計な pane の掃除用途として例外的に扱う.

### 3.6 補助関数

- `tmux-peek-server-running-p-async`: tmux サーバーが起動しているかを真偽値で返す. 同期補助版は `tmux-peek-server-running-p`
- `tmux-peek-version-async`: tmux 自身のバージョン文字列を返す. 同期補助版は `tmux-peek-version`
- `tmux-peek-target-exists-p-async`: 指定ターゲット(`session`, `session:window`, `session:window.pane`)の存在確認. 同期補助版は `tmux-peek-target-exists-p`
...
```elisp
(tmux-peek-list-sessions &optional opts) ;; => (list of plist) or signals error
```

- 同期版は補助 API として扱う
- 実装する場合は非同期 executor を使う薄い wrapper にする
- `accept-process-output` による再入性リスクを考慮し, 初期実装の必須対象にはしない
...
- `tmux-peek-default-format-fields-session`: list-sessions のフォーマットフィールド既定セット
- `tmux-peek-default-format-fields-window`: 同上(window)
- `tmux-peek-default-format-fields-pane`: 同上(pane)
- `tmux-peek-default-format-fields-client`: 同上(client)
- `tmux-peek-default-format-fields-buffer`: 同上(buffer)
...
- `kill-pane` は成功/失敗を非同期 result として返す
...
- **同期層**(`tmux-peek--exec`): 必要になった場合だけ追加する補助 wrapper
...
- enkan-repl 側からは主に以下を利用する想定:
  - `tmux-peek-server-running-p` で tmux 利用可否確認
  - `tmux-peek-list-panes` でアクティブペイン把握
  - `tmux-peek-display-message` で `#{pane_pid}` 等の取得
  - `tmux-peek-capture-pane` / `:tail-lines` で AI CLI 出力の末尾確認
  - enkan-repl だけでは kill しきれない可能性がある余計な pane の確認と `kill-pane`
...
tmux-peek 側では状態確認 API の安定化を優先しつつ, enkan-repl で作られた余計な pane を掃除するための `kill-pane` だけは実装対象に含める.
`kill-session`, `kill-window`, `kill-server` は tmux-peek では封印する.
...
- 自動判定で kill する関数は作らず, 呼び出し側が明示した pane target だけを削除する
- `kill-session`, `kill-window`, `kill-server` は実装しない. 誤用防止のため API として存在させない

### 11.6 第六段階: enkan-repl 連携前提の確認

- enkan-repl の tmux backend が必要とする状態確認 API を棚卸しする
- `list-panes`, `display-message`, `capture-pane`, `server-running-p`, `target-exists-p`, `kill-pane` の不足を確認する
...
- 実 tmux 3.6a で list/display/capture/show/kill-pane 系の動作を確認する
